...
final class HTTPEncoderTest extends TestCase
{
    ...
    public function testZlibEncode()
    {
        ...
        if (!$have_zlib) {
            static::markTestSkipped('Zlib support is not present in PHP. Encoding cannot be performed/tested.');
        }
        ...
        static::assertTrue(isset($headers['Vary']), 'Vary always sent to good browsers');

        $_SERVER['HTTP_USER_AGENT'] = 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)';
        $he = new HTTP_Encoder(array('content' => 'Hello'));
        $he->encode();
        $headers = $he->getHeaders();
        static::assertTrue(!isset($headers['Vary']), 'Vary not sent to bad IE (Issue 126)');
    }
}
